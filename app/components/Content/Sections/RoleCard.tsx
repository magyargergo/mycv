...
const RoleCard: React.FC<RoleCardProps> = ({ role }) => {
    return (
        <div
            className="bg-secondary hover:bg-accent p-5 rounded-lg border-l-4 border-primary shadow-sm transition-all duration-200 hover:shadow-md hover:-translate-y-1"
        >
            ...
            <ul className="space-y-2">
                {role.responsibilities.map((item: string, i: number) => (
                    <li key={i} className="flex text-foreground text-sm">
